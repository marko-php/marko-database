...
describe('NoDriverException', function (): void {
    ...
    it('derives docs URLs from the package basename (marko slash prefix stripped)', function (): void {
        ...
        expect($exception->getSuggestion())->toContain('https://marko.build/docs/packages/database-pgsql/')
            ->and($exception->getSuggestion())->toContain('https://marko.build/docs/packages/database-mysql/');
    });

    it('lists pgsql first in the suggestion (matching known-drivers.php order)', function (): void {
        $exception = NoDriverException::noDriverInstalled();
        $suggestion = $exception->getSuggestion();

        $pgsqlPos = strpos($suggestion, 'marko/database-pgsql');
        $mysqlPos = strpos($suggestion, 'marko/database-mysql');

        expect($pgsqlPos)->toBeLessThan($mysqlPos);
    });
