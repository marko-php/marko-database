 ...
 *
 * @noinspection PhpUnused - Used by generateTestMigration()
 */
function createSqlGeneratorStub(
 ...
 */
function createPostsTableDiff(): SchemaDiff
{
 ...
 * @param array<array<[]> $executedBindings Reference array to track bindings
 */
 ...
 */
function getEmptyMigrationContent(): string
{
    return <<<'PHP'
        <?php
        declare(strict_types=1);
 ...
 */
function writeTestMigrationFiles(
    string $path,
    ?string $content = null,
): array {
    $content ??= getEmptyMigrationContent();
 ...
 */
function generateTestMigration(
    string $tempDir,
    ?SchemaDiff $diff = null,
    array $upStatements = ['CREATE TABLE "posts" (id INT)'],
    array $downStatements = ['DROP TABLE "posts"'],
): array {
    $sqlGenerator = createSqlGeneratorStub($upStatements, $downStatements);
    $generator = new MigrationGenerator($sqlGenerator, $tempDir);
    $diff ??= createPostsTableDiff();
